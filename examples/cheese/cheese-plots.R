...
# Model for part 1
model.pca.1 <- prcomp(cheese[part1,2:4], scale=TRUE)
P.1 <- model.pca.1$rotation
T.1 <- model.pca.1$x
model.pcr.1 <- lm(cheese$Taste[part1] ~ T.1[,1])
# Model for part 2
model.pca.2 <- prcomp(cheese[part2,2:4], scale=TRUE)
...
x.2.new <- scale(cheese[part2,2:4], center= model.pca.1$center, scale=model.pca.1$scale)
x.1.new <- scale(cheese[part1,2:4], center= model.pca.2$center, scale=model.pca.2$scale)
...
# ----------------------------
library(pls)
model.pcr.quick <- mvr(Taste ~ Acetic + H2S + Lactic, data=cheese, method="svdpc", scale=TRUE)
...
# ----------------------------
library(pls)
model.pls <- mvr(Taste ~ Acetic + H2S + Lactic, data=cheese, method="simpls", scale=TRUE)
summary(model.pls)
