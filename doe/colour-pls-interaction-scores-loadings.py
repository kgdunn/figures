...
run's compound, its pH level, and its concentration level. Colour is the chromogen; marker shape is
the pH level (down triangle low, up triangle high); marker size is the concentration level (small
low, large high). Right: the W* factor weights (24 interaction terms) and the C response-point
weights (ten time points) on the same axes. Regenerates
...
wstar = pls.direct_weights_          # X-space weights (24 interaction terms)
...
# Down triangle for low pH (acidic), up triangle for high pH; small marker at low concentration,
# large marker at high concentration. Colour stays the chromogen so all three read at once.
for c, colour in zip(COMPOUND_LEVELS, palette):
    for lo_pH, marker in ((True, "v"), (False, "^")):
...
enc_handles = [
    ...
    Line2D([], [], marker="^", ls="", color="0.35", markeredgecolor="w", markersize=5,
           label="low concentration"),
    Line2D([], [], marker="^", ls="", color="0.35", markeredgecolor="w", markersize=10,
           label="high concentration"),
]
leg1 = axS.legend(handles=colour_handles, frameon=False, fontsize=8, ncol=2, loc="upper left",
                  title="chromogen")
axS.add_artist(leg1)
axS.legend(handles=enc_handles, frameon=False, fontsize=8, loc="lower right")
axS.grid(alpha=0.2)

# --- Panel B: W* (24 interaction terms) and C (ten time points) loadings ---
def short(name):
    return (name.replace("C(compound, Sum)[S.", "cmp").replace("]", "")
            .replace(":", ":").replace("co_solvent", "cosolv"))

axL.scatter(wstar.iloc[:, 0], wstar.iloc[:, 1], s=44, color="#1f5fa8", marker="o",
            edgecolor="w", linewidth=0.5, zorder=3)
for name, (a, b) in zip(wstar.index, wstar.iloc[:, :2].to_numpy()):
    axL.annotate(short(name), (a, b), fontsize=6.5, color="#12406e",
...
axL.set_title("(b) Loadings: W* interaction terms and C response points", fontsize=9.5, loc="left")
axL.legend(handles=[Line2D([], [], marker="o", ls="", color="#1f5fa8", label="model term (W*)"),
...
print("W* term count:", wstar.shape[0])
