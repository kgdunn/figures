...
that are not a foldover design at all. Every value is read from
get_omars_trade_off_table_entry, so the figure cannot drift away from the library.
...
for i, row in enumerate(cells):
    for j, cell in enumerate(row):
        ax.add_patch(Rectangle((j, i), 1, 1, facecolor=FILLS[cell.capability],
                               edgecolor="white", lw=2.0))
        if cell.exists:
            ax.text(j + 0.5, i + 0.36, cell.tag, ha="center", va="center",
                    fontsize=13.5, fontweight="bold", color=INKS[cell.capability])
            ax.text(j + 0.5, i + 0.72, f"df = {cell.error_df}", ha="center", va="center",
                    fontsize=11, color=INKS[cell.capability])
